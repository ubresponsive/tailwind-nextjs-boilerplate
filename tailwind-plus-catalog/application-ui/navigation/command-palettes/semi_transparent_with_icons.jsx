...
export default function NavigationCommandPalettesSemiTransparentWithIcons() {
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        <DialogPanel
          transition
          className="mx-auto max-w-2xl transform divide-y divide-gray-500/10 overflow-hidden rounded-xl bg-white/80 shadow-2xl outline-1 outline-black/5 backdrop-blur-sm backdrop-filter transition-all data-closed:scale-95 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"
        >
          <Combobox
            onChange={(item) => {
              if (item) {
                  ...
                  <ul className="text-sm text-gray-700">
                    {(query === '' ? recent : filteredProjects).map((project) => (
                      <ComboboxOption
                        ...
                        key={project.id}
                        value={project}
                        className="group flex cursor-default items-center rounded-md px-3 py-2 select-none data-focus:bg-gray-900/5 data-focus:text-gray-900 data-focus:outline-hidden"
                      >
                        <FolderIcon
                        ...
                        <span className="ml-3 hidden flex-none text-gray-500 group-data-focus:inline">Jump to...</span>
                      </ComboboxOption>
                    ))}
                  </ul>
                </li>
