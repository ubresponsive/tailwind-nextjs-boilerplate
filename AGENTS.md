...
- **Forms use Server Actions**, not `useState` + `fetch` to a hand-rolled API route. Validate input
  with **Zod** on the server. See `src/app/contact/`.
...
- **Form dropdowns must use `src/components/ui/select-field.tsx`.** Do not add raw `<select>` markup
  in page or form components. The shared `SelectField` removes the native browser chevron, adds the
  project chevron icon, and keeps focus/error styling consistent across forms.
